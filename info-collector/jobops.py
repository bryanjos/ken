...
def convert_form_to_job(job_form):
    return {
        ...
        'sources': job_form.getlist('sources'),
        'time': job_form['time'].strip(),
        'location': job_form['location'].strip() if len(job_form['location'].strip()) > 0 else None,
...
def get_job(job_slug):
    ...
    return Job(job.name, job.slug, job.time, job.tags, job.location, job.lat, job.lon, job.distance)
...
def get_jobs():
    ...
    for key in keys:
        job = bucket.get(key).get_data()
        jobs.append(Job(job.name, job.slug, job.time, job.tags, job.location, job.lat, job.lon, job.distance))
